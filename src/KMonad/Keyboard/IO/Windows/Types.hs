...
import qualified RIO.NonEmpty as N (groupAllWith)
import qualified Data.Foldable as N (minimumBy, maximumBy)
...
winCodeToKeyCode = M.fromList $ N.minimumBy (compare `on` snd) <$> N.groupAllWith fst winCodeKeyCodeMapping
...
keyCodeToWinCode = M.fromList $ swap . N.maximumBy (compare `on` fst) <$> N.groupAllWith snd winCodeKeyCodeMapping
